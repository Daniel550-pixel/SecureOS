...
export const ModulePipelineOverview: React.FC<ModulePipelineOverviewProps> = ({
  ...
  const modules = [
    ...
    },
    {
      ...
      countLabel: `${(trustScore?.current_score ?? 100).toFixed(1)}% score`,
      icon: ShieldAlert,
      color: (trustScore?.current_score ?? 100) >= 90 ? 'text-emerald-400' : 'text-rose-400',
    ...
    },
    {
      ...
      countLabel: aiAnalysis ? `${(((typeof aiAnalysis.confidence === 'number' ? aiAnalysis.confidence : aiAnalysis.confidence_score) ?? 0.94) * 100).toFixed(0)}% conf` : 'Ready',
      icon: BrainCircuit,
