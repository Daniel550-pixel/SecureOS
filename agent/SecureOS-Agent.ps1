...
function Send-SecureOSPayload {
  ...
  try {
    ...
    Write-Warning "[SecureOS] telemetry delivery failed: $($_.Exception.Message)"
  }
}
...
function Collect-Telemetry {
  ...
  Get-Process | Select-Object -First $ProcessSampleLimit | ForEach-Object {
    ...
    if ($path -and (Test-Path $path)) {
      try { $signed = ((Get-AuthenticodeSignature -FilePath $path).Status -eq 'Valid') } catch {}
    }
    Send-SecureOSPayload -Category 'process' -Payload @{
      Id = $_.Id
      Name = $_.ProcessName
      Path = $path
      IsSigned = $signed
  ...
  # Security-critical service state
  @('WinDefend', 'EventLog', 'CryptSvc', 'MpsSvc') | ForEach-Object {
    $service = Get-Service -Name $_
    if ($service) {
      Send-SecureOSPayload -Category 'service' -Payload @{
        Name = $service.Name
        Status = $service.Status.ToString()
        StartType = try { (Get-CimInstance Win32_Service -Filter "Name='$($service.Name)'").StartMode } catch { 'Unknown' }
      }
    }
  }
  ...
  Get-NetTCPConnection -State Established | Select-Object -First 100 | ForEach-Object {
    Send-SecureOSPayload -Category 'net_conn' -Payload @{
  ...
  # Host-file integrity observation. No local baseline is created or modified.
  $hostsPath = Join-Path $env:SystemRoot 'System32\drivers\etc\hosts'
  if (Test-Path $hostsPath) {
    $hash = (Get-FileHash -Path $hostsPath -Algorithm SHA256).Hash
    Send-SecureOSPayload -Category 'fim' -Payload @{
      Path = $hostsPath
      HashSHA256 = $hash
...
try {
  while ($Timer.Enabled) { Start-Sleep -Seconds 1 }
} finally {
  $Timer.Stop()
  $Timer.Dispose()
}
