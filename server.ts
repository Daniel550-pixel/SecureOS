...
import { normalizePowerShellPayload, validateNormalizedEvent } from './src/engine/telemetryNormalizer.ts';
import { appendTelemetry, appendTrustSnapshot, appendLifecycleRecord, getLedger, verifyLedger } from './src/engine/eventLedger.ts';
...
const AGENT_TOKEN = process.env.SECUREOS_AGENT_TOKEN;
...
function buildDeterministicAnalysis(deviations: ReturnType<typeof compareAgainstBaseline>, trustScoreResult: ReturnType<typeof calculateTrustScore>, events: NormalizedEvent[] = []): AIIntegrityAnalysis {
  ...
  const correlated = deviations.map((d, idx) => ({ event_id: `CORR-DEV-${idx + 1}`, stage: d.category.includes('net') ? 'Anomalous Egress Communication' : d.category.includes('sec') ? 'Security Control Impairment' : d.category.includes('proc') ? 'Unauthorized Binary Execution' : 'Subsystem Integrity Deviation', title: d.title || `${d.category} anomaly`, description: d.description, involved_entities: [d.entity], mitre_tactic: d.category.includes('net') ? 'Command and Control' : d.category.includes('proc') ? 'Execution' : 'Defense Evasion', mitre_technique: d.category.includes('net') ? 'T1071.001' : d.category.includes('proc') ? 'T1204.002' : 'T1562.001', threat_likelihood: (d.severity === 'critical' ? 'critical' : d.severity === 'high' ? 'high' : 'moderate') as any, evidence: `Expected: "${d.baseline_expected}" vs Observed: "${d.observed_actual}"` }));
  ...
  return { assessment, risk_level: trustScoreResult.risk_classification, explanation, detailed_explanation: explanation, summary_headline: assessment, threat_hypothesis: assessment, attack_narrative: explanation, correlated_events: correlated, correlated_chain: correlated, correlated_activity_chain: correlated, mitre_mappings: correlated.map(ev => ({ technique_id: ev.mitre_technique || 'T1059', tactic: ev.mitre_tactic || 'Execution', technique_name: ev.title, evidence: ev.evidence || ev.description })), recommended_actions: recommended, confidence: isClean ? 0.99 : 0.92, confidence_score: isClean ? 0.99 : 0.92, trust_score_invariant: { ground_truth_score: trustScoreResult.current_score, authority: 'DETERMINISTIC_TRUST_ENGINE', gemini_authority: 'READ_ONLY_ADVISORY', can_alter_trust_score: false, message: 'CRITICAL INVARIANT: Gemini has zero authority to alter the Trust Score. The deterministic Trust Engine is authoritative.' }, generated_at: new Date().toISOString(), model_used: 'Deterministic Integrity Reasoning Engine (Ground Truth Fallback)' };
}
...
app.get('/api/health', (_req, res) => res.json({ status: 'ok', system: 'Digital Integrity & Trust Monitor AIOS Core', version: '2.6.0-INGESTION-HARDENED', has_gemini_key: Boolean(process.env.GEMINI_API_KEY), agent_auth_required: Boolean(AGENT_TOKEN), ledger: verifyLedger() }));
app.get('/api/state', (_req, res) => res.json(stateEnvelope()));
app.get('/api/ledger', (req, res) => { const limit = Math.min(500, Math.max(1, Number(req.query.limit || 100))); res.json({ success: true, verification: verifyLedger(), records: getLedger(limit) }); });
...
app.post('/api/telemetry/ingest', (req, res) => {
  try {
    if (AGENT_TOKEN && req.get('x-secureos-agent-token') !== AGENT_TOKEN) return res.status(401).json({ error: 'telemetry authentication required' });
    const contentLength = Number(req.get('content-length') || 0);
    if (contentLength > MAX_TELEMETRY_BYTES) return res.status(413).json({ error: 'telemetry payload too large' });
    const rawPayload = req.body || {};
    if (!rawPayload || typeof rawPayload !== 'object' || Array.isArray(rawPayload)) return res.status(400).json({ error: 'telemetry payload must be an object' });
    if (rawPayload.collector === 'powershell_agent' || rawPayload.category) {
      const normalizedEvent = normalizePowerShellPayload(rawPayload);
      if (rawPayload.host_id && rawPayload.host_id !== activeState.host_id) return res.status(403).json({ error: 'telemetry host_id does not match active host' });
      activeState.recent_events.unshift(normalizedEvent);
      activeState.recent_events = activeState.recent_events.slice(0, 50);
      const { deviations, trustScore } = currentEvaluation();
      currentAiAnalysis = buildDeterministicAnalysis(deviations, trustScore, activeState.recent_events);
      appendTelemetry(normalizedEvent, activeState.host_id, { scenario_id: currentScenarioId, transport: 'http', collector: 'powershell_agent', authenticated: Boolean(AGENT_TOKEN) });
      ...
      return res.json({ success: true, event_id: normalizedEvent.event_id, ledger_sequence: snapshot.sequence, ...stateEnvelope() });
    }
    if (rawPayload.event_id && rawPayload.entity) {
      validateNormalizedEvent(rawPayload);
      activeState.recent_events.unshift(rawPayload as NormalizedEvent);
      activeState.recent_events = activeState.recent_events.slice(0, 50);
      const { deviations, trustScore } = currentEvaluation();
      currentAiAnalysis = buildDeterministicAnalysis(deviations, trustScore, activeState.recent_events);
      appendTelemetry(rawPayload as NormalizedEvent, activeState.host_id, { scenario_id: currentScenarioId, transport: 'http', collector: 'normalized_event', authenticated: Boolean(AGENT_TOKEN) });
      const snapshot = appendTrustSnapshot(activeState.host_id, trustScore.current_score, trustScore.risk_classification, trustScore.formula_version, { source: 'telemetry_ingest', event_id: rawPayload.event_id });
      return res.json({ success: true, event_id: rawPayload.event_id, ledger_sequence: snapshot.sequence, ...stateEnvelope() });
    }
    return res.status(400).json({ error: 'unsupported telemetry payload shape' });
  } catch (err: any) {
    return res.status(400).json({ error: err?.message || 'Invalid ingestion payload' });
  }
});
...
app.post('/api/actions/execute', (req, res) => {
  ...
  else return res.status(400).json({ error: 'unsupported action_type' });
  const { deviations, trustScore } = currentEvaluation();
  currentAiAnalysis = buildDeterministicAnalysis(deviations, trustScore, activeState.recent_events);
