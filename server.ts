...
function buildDeterministicAnalysis(
  ...
  const explanation = isClean
    ? 'All host running processes match digital signature white-lists. Crucial security daemons are running with nominal hashes. No suspicious network sockets or file integrity anomalies detected.'
    : `Deterministic baseline verification identified ${deviations.length} baseline violations resulting in an authoritative Trust Score of ${trustScoreResult.current_score}%. Telemetry correlation indicates a causal sequence across ${deviations.map(d => d.entity).slice(0, 3).join(', ')}.`;
  ...
  if (isClean) correlated.push({
    event_id: 'CORR-EV-01', stage: 'Nominal Baseline Verification', title: 'All Core Subsystems Signed and Validated',
    description: 'Host telemetry confirms zero unauthorized baseline deviations.', involved_entities: ['ntoskrnl.exe', 'services.exe', 'WinDefend'],
    mitre_tactic: 'Discovery', mitre_technique: 'T1082', threat_likelihood: 'low' as const,
    evidence: 'Verified against deterministic baseline'
  });
  const recommended = isClean ? [{
    id: 'ACT-01', title: 'Maintain Real-Time Telemetry Streaming', action: 'Maintain Real-Time Telemetry Streaming', type: 'hardening' as const, priority: 'low' as const, urgency: 'LOW' as const,
    command_snippet: 'Get-Service AIOS-Telemetry | Select-Object Status, StartType', command: 'Get-Service AIOS-Telemetry | Select-Object Status, StartType',
    explanation: 'Ensure continuous telemetry coverage.', rationale: 'Maintain monitoring coverage.'
  }] : [{
    id: 'ACT-01', title: 'Isolate Host from Sovereign Network Fabric', action: 'Isolate Host from Sovereign Network Fabric', type: 'containment' as const, priority: 'urgent' as const, urgency: 'IMMEDIATE' as const,
    command_snippet: 'Set-NetFirewallProfile -Profile Domain,Public,Private -Enabled True', command: 'Set-NetFirewallProfile -Profile Domain,Public,Private -Enabled True',
    explanation: 'Contain lateral movement and suspicious egress while preserving telemetry.', rationale: 'Reduce active attack surface.'
  }, {
    id: 'ACT-02', title: 'Restore Critical Security Services', action: 'Restore Critical Security Services', type: 'remediation' as const, priority: 'high' as const, urgency: 'HIGH' as const,
    command_snippet: 'Set-Service -Name WinDefend -StartupType Automatic; Start-Service WinDefend', command: 'Set-Service -Name WinDefend -StartupType Automatic; Start-Service WinDefend',
    explanation: 'Restore endpoint security controls.', rationale: 'Re-enable defensive telemetry.'
  }];
  return {
    assessment, risk_level: trustScoreResult.risk_classification, explanation, detailed_explanation: explanation,
    summary_headline: assessment, threat_hypothesis: assessment, attack_narrative: explanation,
    ...
    trust_score_invariant: {
      ...
      authority: 'DETERMINISTIC_TRUST_ENGINE', gemini_authority: 'READ_ONLY_ADVISORY', can_alter_trust_score: false,
      message: 'CRITICAL INVARIANT: Gemini has zero authority to alter the Trust Score. The deterministic Trust Engine is authoritative.'
    },
...
function stateEnvelope() {
  const { deviations, trustScore } = currentEvaluation();
  return {
    scenario_id: currentScenarioId, scenario: SCENARIOS.find(s => s.id === currentScenarioId) || SCENARIOS[0], state: activeState,
    baseline: DEFAULT_BASELINE, deviations, trust_score: trustScore, trustScore,
    ai_analysis: currentAiAnalysis, aiAnalysis: currentAiAnalysis, analysis: currentAiAnalysis, timestamp: new Date().toISOString()
  };
}

app.get('/api/health', (_req, res) => res.json({
  status: 'ok', system: 'Digital Integrity & Trust Monitor AIOS Core', version: '2.5.0-PERSISTENT-LEDGER',
  has_gemini_key: Boolean(process.env.GEMINI_API_KEY), ledger: verifyLedger()
}));
...
app.get('/api/ledger', (req, res) => {
  ...
  res.json({ success: true, verification: verifyLedger(), records: getLedger(limit), ledger_path: getLedgerPath() });
});

app.get('/api/trust/history', (req, res) => {
  const limit = Math.min(1000, Math.max(1, Number(req.query.limit || 200)));
  const records = getLedger(limit).filter(record => record.record_type === 'trust_snapshot');
  res.json({ success: true, records, verification: verifyLedger() });
});
...
app.post('/api/telemetry/ingest', (req, res) => {
  try {
    ...
    if (rawPayload.collector === 'powershell_agent' || rawPayload.category) normalizedEvent = normalizePowerShellPayload(rawPayload);
      ...
      event_id: `EV-INGEST-${Date.now()}`, timestamp: new Date().toISOString(), source: 'edr_sensor',
      type: rawPayload.type || 'process_start', severity: rawPayload.severity || 'medium', entity: rawPayload.entity || 'Injected Entity',
      evidence: rawPayload.evidence || rawPayload, confidence: 0.95, mitre_tactic: rawPayload.mitre_tactic || 'Execution', mitre_technique: rawPayload.mitre_technique || 'T1059'
    };
    ...
    if (rawPayload.action === 'add_process' && rawPayload.process) activeState.processes.push(rawPayload.process);
      ...
      const svc = activeState.services.find(s => s.name === rawPayload.service_name); if (svc) svc.status = 'Stopped';
    } else if (rawPayload.action === 'modify_fim' && rawPayload.file_path) {
      const fim = activeState.fim_files.find(f => f.path === rawPayload.file_path); if (fim) { fim.is_modified = true; fim.current_hash = `tampered_${Date.now().toString(16)}`; }
    }

    const { deviations, trustScore } = currentEvaluation();
    currentAiAnalysis = buildDeterministicAnalysis(deviations, trustScore, activeState.recent_events);
    appendTelemetry(normalizedEvent, activeState.host_id, { scenario_id: currentScenarioId, transport: 'http' });
...
app.post('/api/actions/execute', (req, res) => {
  ...
  if (action_type === 'revert_baseline') {
    const clean = SCENARIOS.find(s => s.id === 'clean_baseline')!; currentScenarioId = clean.id; activeState = JSON.parse(JSON.stringify(clean.state));
  } else if (action_type === 'kill_process' || action_type === 'terminate_process') {
    activeState.processes = activeState.processes.filter(p => !p.name.includes(target_entity || '') && String(p.pid) !== String(target_entity || ''));
  } else if (action_type === 'restart_service' || action_type === 'restore_service') {
    const svc = activeState.services.find(s => s.name === target_entity || s.display_name === target_entity); if (svc) svc.status = 'Running';
  } else if (action_type === 'block_network' || action_type === 'isolate_host') {
    activeState.network_connections = target_entity ? activeState.network_connections.filter(c => c.remote_ip !== target_entity && !target_entity.includes(c.remote_ip)) : [];
  } else if (action_type === 'restore_file' || action_type === 'restore_fim') {
    const fim = activeState.fim_files.find(f => f.path.includes(target_entity || '')); if (fim) { fim.current_hash = fim.expected_hash; fim.is_modified = false; }
  }
  const { deviations, trustScore } = currentEvaluation();
  currentAiAnalysis = buildDeterministicAnalysis(deviations, trustScore, activeState.recent_events);
...
app.post('/api/gemini/analyze', async (req, res) => {
  ...
  if (!ai) {
    const fallback = buildDeterministicAnalysis(deviations, trustScore, events); currentAiAnalysis = fallback;
    return res.json({ success: true, analysis: fallback, aiAnalysis: fallback, ...fallback });
  }
  try {
    ...
    const response = await ai.models.generateContent({
      ...
      config: { responseMimeType: 'application/json', responseSchema: {
        type: Type.OBJECT, properties: {
          assessment: { type: Type.STRING }, explanation: { type: Type.STRING },
          correlated_events: { type: Type.ARRAY, items: { type: Type.OBJECT, properties: {
            event_id: { type: Type.STRING }, stage: { type: Type.STRING }, title: { type: Type.STRING }, description: { type: Type.STRING },
            involved_entities: { type: Type.ARRAY, items: { type: Type.STRING } }, mitre_tactic: { type: Type.STRING }, mitre_technique: { type: Type.STRING },
            threat_likelihood: { type: Type.STRING }, evidence: { type: Type.STRING }
          }, required: ['stage', 'title', 'description', 'involved_entities', 'threat_likelihood'] } },
          recommended_actions: { type: Type.ARRAY, items: { type: Type.OBJECT, properties: {
            id: { type: Type.STRING }, title: { type: Type.STRING }, type: { type: Type.STRING }, priority: { type: Type.STRING }, command_snippet: { type: Type.STRING }, explanation: { type: Type.STRING }
    ...
    const correlated = (parsed.correlated_events || []).map((ev: any, i: number) => ({
      ...
      involved_entities: Array.isArray(ev.involved_entities) ? ev.involved_entities : [], mitre_tactic: ev.mitre_tactic || 'Execution', mitre_technique: ev.mitre_technique || 'T1059',
      threat_likelihood: ev.threat_likelihood || 'moderate', evidence: ev.evidence || ev.description || 'Observed via telemetry'
    }));
    const actions = (parsed.recommended_actions || []).map((act: any, i: number) => ({
      id: act.id || `ACT-${String(i + 1).padStart(2, '0')}`, title: act.title || 'Recommended Mitigation', action: act.title || 'Recommended Mitigation', type: act.type || 'investigation', priority: act.priority || 'medium',
      urgency: act.priority === 'urgent' ? 'IMMEDIATE' : act.priority === 'high' ? 'HIGH' : 'MEDIUM', command_snippet: act.command_snippet || '', command: act.command_snippet || '', explanation: act.explanation || '', rationale: act.explanation || ''
    }));
    currentAiAnalysis = {
      assessment: parsed.assessment || trustScore.status_label, risk_level: trustScore.risk_classification, explanation: parsed.explanation || '', detailed_explanation: parsed.explanation || '',
      summary_headline: parsed.assessment || trustScore.status_label, threat_hypothesis: parsed.assessment || 'Telemetry correlation', attack_narrative: parsed.explanation || '',
      correlated_events: correlated, correlated_chain: correlated, correlated_activity_chain: correlated,
      mitre_mappings: correlated.map((ev: any) => ({ technique_id: ev.mitre_technique || 'T1059', tactic: ev.mitre_tactic || 'Execution', technique_name: ev.title, evidence: ev.evidence || ev.description })),
      recommended_actions: actions, confidence: typeof parsed.confidence === 'number' ? parsed.confidence : 0.9, confidence_score: typeof parsed.confidence === 'number' ? parsed.confidence : 0.9,
      trust_score_invariant: { ground_truth_score: trustScore.current_score, authority: 'DETERMINISTIC_TRUST_ENGINE', gemini_authority: 'READ_ONLY_ADVISORY', can_alter_trust_score: false, message: 'Gemini is advisory only; Trust Engine remains authoritative.' },
      generated_at: new Date().toISOString(), model_used: 'gemini-3.8-flash'
    };
    res.json({ success: true, analysis: currentAiAnalysis, aiAnalysis: currentAiAnalysis, ...currentAiAnalysis });
  } catch (err) {
    const fallback = buildDeterministicAnalysis(deviations, trustScore, events); currentAiAnalysis = fallback;
    res.json({ success: true, analysis: fallback, aiAnalysis: fallback, ...fallback });
  }
});
